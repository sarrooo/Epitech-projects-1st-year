...
void load_dark_step2(game_t *game, char* filepath)
{
    ...
    sfSprite_setTexture(game->map.data_htp->dark,
    game->map.data_htp->t_dark, sfTrue);
}

void event_handler_htp01(game_t *game)
{
    if (game->event.type == sfEvtKeyPressed &&
    game->event.key.code == sfKeyEnter) {
        if (game->map.data_htp->finished == 1) {
            game->handler_scene = &game_display_htp2;
            free(game->map.data_htp->temp);
            free(game->map.data_htp->str);
            read_dialogue(game, "./htp/dialogue_1.txt");
            load_dark_step2(game, "./asset/bg/dark_fire.png");
...
void event_handler_htp0(game_t *game)
{
...
void display_typing_0(game_t *game)
{
    game->map.time = game->map.time_base;
    game->map.time += get_time(game);
    if (game->map.data_htp->str[game->map.data_htp->i] == '\0') {
        game->map.data_htp->finished = 1;
        sfSound_stop(game->map.data_htp->tping);
    }
    if (game->map.data_htp->finished != 1 &&
    game->map.data_htp->current_timer + 0.08 < game->map.time) {
        game->map.data_htp->temp[game->map.data_htp->i] =
        game->map.data_htp->str[game->map.data_htp->i];
        game->map.data_htp->current_timer = game->map.time;
        game->map.data_htp->i += 1;
        game->map.data_htp->temp[game->map.data_htp->i] = '\0';
        sfText_setString(game->map.data_htp->text, game->map.data_htp->temp);
    }
}

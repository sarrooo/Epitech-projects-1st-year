...
void event_handler_htp21(game_t *game)
{
...
void event_handler_htp2(game_t *game)
{
    while (sfRenderWindow_pollEvent(game->win, &game->event)) {
        ...
        event_handler_htp21(game);
    }
}

void activate_freeze(game_t *game)
{
    game->map.time = game->map.time_base;
    game->map.time += get_time(game);
    sfIntRect my_rect =
    sfSprite_getTextureRect(game->map.fireball.sprite_spell);

    if (game->map.data_htp->started != 1 && my_rect.left > 2400) {
        game->map.data_htp->started = 1;
        game->map.data_htp->titime = game->map.time;
    }
}

void go_to_freeze(game_t *game)
{
    game->map.time = game->map.time_base;
    game->map.time += get_time(game);
    if (game->map.data_htp->started == 1 &&
    game->map.data_htp->titime + 1 < game->map.time) {
        game->handler_scene = &game_display_htp3;
        free(game->map.data_htp->temp);
        free(game->map.data_htp->str);
        read_dialogue(game, "./htp/dialogue_2.txt");
        load_dark_step2(game, "./asset/bg/freeze_png.png");

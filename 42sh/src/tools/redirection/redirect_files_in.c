...
static int manage_double_input(head_t *head, cmd_list_t *cmd)
{
    ...
    while (strcmp(temp, cmd->input_redirection) != 0 &&
    strcmp_without_jump_line(temp, cmd->input_redirection) != 0) {
        free(temp);
        temp = NULL;
        write(1, "? ", 2);
        getline(&temp, &len, stdin);
        if (strcmp(temp, cmd->input_redirection) != 0 &&
        strcmp_without_jump_line(temp, cmd->input_redirection) != 0) {
            str = realloc(str, sizeof(char) * (strlen(str) + strlen(temp) + 1));
            strcat(str, temp);
        }
    }
    return (finish_double_input(cmd, temp, str));
}
